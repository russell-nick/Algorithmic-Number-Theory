...
mp::mpz_int mod(mp::mpz_int a, mp::mpz_int n) {
    if (n == 0) return a;
    mp::mpz_int result = a - n * (a/n); // quotient = a / n; return a - n * quotient
...
mp::mpz_int extended_gcd(mp::mpz_int a, mp::mpz_int b, mp::mpz_int &x, mp::mpz_int &y) {
    ...
    mp::mpz_int x1 = 0, y1 = 1, a1 = a, b1 = b;
    mp::mpz_int quotient, tmp;
    while (b1 != 0) {
        quotient = a1 / b1;
...
//
//    mp::mpz_int x1, y1;
//    mp::mpz_int d = extended_gcd(b, a % b, x1, y1);
//
//    x = y1;
//    y = x1 - y1 * (a / b);
//    return d;
...
bool miller_rabin(mp::mpz_int n, int k) {
    ...
    for (int i = 0; i < k; i++) {
        ...
        std::uniform_int_distribution<mp::mpz_int> distribution(2, n-2); // or [1, n-1] also works?
        mp::mpz_int a = distribution(generator);
